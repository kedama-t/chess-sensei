...
LLM に手を選ばせないので、非合法手や的外れな読み筋が出ません。エンジンの分析は常に
表示され、LLM を有効にした場合はその下に日本語の説明文が追加されます（LLM 無効でも
アプリはそれだけで成立します）。
...
- **感想戦（棋譜レビュー）** — 対局後（途中でも）棋譜全体を解析し、白黒それぞれの
  精度(%)・平均損失(ACPL)・判定内訳を集計。手をクリックするとその局面を盤に再現して
  講評を表示し、「ここから指し直す」でその局面から対局を再開できる
- **AI 解説（任意）** — 上記の解析結果だけを根拠に、Gemma がやさしい日本語で説明。
  コーチタブは「♟ Stockfish の分析」と「🗣 AI 解説」の 2 段構成で、AI 解説を有効にしても
  エンジンの分析（評価値・最善手・読み筋）は常に表示されます
- **無制限の「まった」／棋譜／解析タブ**
...
- UCI ラッパーは `src/engine/uci.ts`、評価値の正規化・悪手判定は `src/engine/analysis.ts`、
  棋譜全体のレビューは `src/engine/gameReview.ts`
- 精度(%)は Lichess と同じ式（勝率変換 + 減衰）で算出しています
  （[Lichess: Accuracy](https://lichess.org/page/accuracy)）
- 棋譜 N 手のレビューでも解析は N+1 局面ぶん（局面 i の解析が「手 i の前」と
  「手 i-1 の後」を兼ねる）
...
- プロンプトでは「解析結果にない手・評価を書かない」ことを明示し、生成が失敗しても
  Stockfish の分析だけが表示されます
- 小さいオンデバイス LLM は SAN と評価値だけでは理由を語れないため、`src/engine/features.ts`
  で盤面から具体的な事実を機械的に抽出してプロンプトに渡しています
  - ただ取りされそうな駒（攻め駒と守り駒の枚数・価値による近似）
  - その手で新たに当たりにした相手の駒
  - 読み筋の日本語化（`Nxe5` →「ナイトで e5 のポーンを取る」）
  - 駒の損得・展開したマイナーピース数・キングの位置とキャスリング・中央の駒と効き
